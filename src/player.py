...
class Player(Entity):
    def __init__(
        self, pos, groups, obstacles, create_attack, kill_attack, use_item, kill_item
    ):
        super().__init__(groups)
        self.image = pygame.image.load(
            os.path.join("graphics", "character", "player", "down_idle", "0.png")
        ).convert_alpha()
    ...
    def input(self):
        ...
        if not self.attacking:
            ...
            if (
                ...
                and self.stamina >= self.stamina_cost
            ):
                self.attacking = True
